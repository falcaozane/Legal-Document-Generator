  ...
  return (
    ...
    ? messages.map((m) => (
      <div key={m.id} className="whitespace-pre-wrap mx-2">
        {m.role === "user" ? <p className="font-semibold text-normal text-[#1A63F1]">You</p> : <p className="text-[#3aad6e] font-semibold">AI</p>}
        {m.role === "user" ? (
          ...
          <>
            <div contentEditable='true' className="rounded-xl focus:outline-none focus:ring-2 focus:ring-lime-900/10">
              <p className="text-justify mt-2 px-5 py-8 font-normal text-sm bg-green-50 rounded-lg" id={m.id}>{m.content}</p>
            </div>
            ...
            <div className="my-6 px-2">
              ...
              <button className="bg-[#19BA92] hover:bg-[#058f6d] text-white mx-2 font-semibold text-sm py-2 px-4 rounded-lg" onClick={
                  () => {
                    const element = document.getElementById(m.id);
                    if (element) {
              ...
              <button className="bg-[#19BA92] hover:bg-[#058f6d] text-white font-semibold text-sm py-2 px-4 rounded-lg " onClick={
                  () => {
                    const element = document.getElementById(m.id);
                    if (element) {
    ...
    : null}
      ...
      <div className="lower-div bg-white z-50 bg-fixed mx-auto w-full max-w-4xl h-20">
        <form onSubmit={handleSubmit} className="">
          <div className="">
            <input
              className="fixed h-12 w-full rounded-xl py-3 px-4 text-sm max-w-4xl bottom-0 border border-gray-300  mb-8 
